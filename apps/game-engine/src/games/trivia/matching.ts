...
// only defense against false negatives from typos and formatting, with the dispute vote as the
// human fallback.
//
// Normalization (order matters):
//   1. lowercase
//   2. drop punctuation (anything that is not a letter, number, or whitespace)
//   3. collapse inner whitespace to single spaces and trim
//   4. strip a single leading article (`a` / `an` / `the`)
//   5. collapse/trim again
//
...
// different word). Short answers require an exact normalized match to avoid "cat" ~ "cot".
