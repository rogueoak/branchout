...
import { cellAt, cellBox, layoutBoard, readCssVar } from './geometry';

// Pure geometry tests for the game-agnostic board renderer (spec 0054). The canvas draw loop is not
// jsdom-runnable, but the layout + hit-test math is - and it is the correctness-critical part (a tap
// must map to the same cell the draw loop paints). This math is shared by Reversi/Checkers/Chess.
...
describe('readCssVar', () => {
  it('returns the fallback with no element (SSR / first paint)', () => {
    expect(readCssVar(null, '--anything', '#123456')).toBe('#123456');
  });
});
