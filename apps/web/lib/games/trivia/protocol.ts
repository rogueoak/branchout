// Trivia's payload decoders for the game-pluggable client (spec 0023). The protocol (spec 0007)
// types the envelope but leaves `prompt`/`reveal` as `unknown` - each game defines their shape
// (spec 0008, packages/games/trivia). These guards turn the opaque field into a typed value at the
// client boundary, so a shape the UI does not understand is a null (a skipped render), never a
// thrown render. The `pick*` helpers scan the reducer's opaque `reveals: unknown[]` list - a round
