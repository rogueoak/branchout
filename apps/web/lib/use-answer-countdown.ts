...
// The visible answer countdown (spec 0017). The engine sends the time *remaining* in each `state`
...
/**
 * Seconds left in the answer window, or null when there is no timer. While paused it holds at the
...
export function useAnswerCountdown(
  ...
  useEffect(() => {
    setDeadline(answerMsRemaining == null ? null : Date.now() + answerMsRemaining);
  }, [answerMsRemaining, round, paused]);
  ...
  if (answerMsRemaining == null) return null;
  // Paused: the engine's clock is stopped, so show the frozen remaining, not the (stale) deadline.
  if (paused) return Math.ceil(answerMsRemaining / 1000);
  if (deadline == null) return Math.ceil(answerMsRemaining / 1000);
