// Liar Liar host configuration and its validator (spec 0021). The host picks 1-3 categories (or
// `random` across all) and a round count. `validateConfig` is the plugin manifest's config schema:
...
/** The sentinel that draws clues across every category. */
export const RANDOM = 'random';

/** Host-supplied configuration, before validation/defaulting. */
export interface LiarLiarConfig {
...
export interface ResolvedLiarLiarConfig {
  categories: string[] | 'random';
  rounds: number;
}
 ...
 */
export function validateConfig(config: unknown): ResolvedLiarLiarConfig {
  const cfg = (config ?? {}) as LiarLiarConfig;
  ...
  if (cfg.categories === RANDOM) {
    ...
    if (chosen.length < MIN_CATEGORIES || chosen.length > MAX_CATEGORIES) {
      throw new Error(
        `liar-liar categories must be '${RANDOM}' or an array of ${MIN_CATEGORIES}-${MAX_CATEGORIES} ` +
          `categories, got ${chosen.length}`,
      );
    }
    if (new Set(chosen).size !== chosen.length) {
      throw new Error(`liar-liar categories must be distinct, got ${JSON.stringify(chosen)}`);
    }
    for (const category of chosen) {
      if (typeof category !== 'string' || !known.has(category)) {
        throw new Error(
          `liar-liar category ${JSON.stringify(category)} is unknown; expected one of ` +
            CATEGORIES.join(', '),
        );
      }
    }
    ...
    throw new Error(
      `liar-liar categories must be '${RANDOM}' or an array of ${MIN_CATEGORIES}-${MAX_CATEGORIES} of ` +
  ...
  if (!Number.isInteger(rounds) || rounds < MIN_ROUNDS || rounds > MAX_ROUNDS) {
    throw new Error(
      `liar-liar rounds must be an integer ${MIN_ROUNDS}-${MAX_ROUNDS}, got ${JSON.stringify(rounds)}`,
    );
  }
